...
La consulta tiene cuatro secciones desplegables en un mismo desplazamiento: S — Subjetivo (lo que refiere el paciente), O — Objetivo (hallazgos y mediciones), A — Análisis (valoración y diagnóstico), P — Plan (tratamiento e indicaciones). S empieza lista para escribir; los accesos S/O/A/P permanecen visibles y abren su sección sin cerrar las demás. Cada encabezado muestra un resumen de lo registrado. Plegar conserva los mismos controles, textos, selección y elementos estructurados; el estado abierto/cerrado se guarda con el borrador.

O incluye Añadir signos vitales, Registrar exploración y Ver evolución. A permite Añadir diagnóstico o impresión y escribir valoración libre. P contiene indicaciones, medicamentos y Opciones de tratamiento y estudios (medicación previa, receta y estudios). Documentos de esta consulta muestra el destino, el número incorporado y los pendientes. Las notas anteriores conservan sus campos completos sin reinterpretarlos ni rellenar hallazgos automáticamente.

El encabezado conserva paciente, folio, edad, doctor, fecha, estado y alergias. Editar detalles abre fecha, hora y tipo. Ampliar aumenta el mismo editor sin sustituirlo. Ctrl+S guarda; Ctrl+Enter abre la revisión de Finalizar consulta. Solo hay una captura activa; cambiar de paciente espera a resolverla.
